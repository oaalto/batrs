...
impl TriadGuild {
    ...
    fn cast_cause_light_wounds(
        ...
        Some(cast_spell("cause light wounds", data))
    }
    ...
    fn cast_cause_serious_wounds(
        ...
        Some(cast_spell("cause serious wounds", data))
    }
    ...
    fn cast_cause_critical_wounds(
        ...
        Some(cast_spell("cause critical wounds", data))
    }

    fn cast_damn_armament(_: &command::Data, _: &mut command::CommandContext) -> Option<String> {
    ...
    fn cast_mellon_collie(data: &command::Data, _: &mut command::CommandContext) -> Option<String> {
        Some(cast_spell("mellon collie", data))
    }

    fn cast_aura_of_hate(_: &command::Data, _: &mut command::CommandContext) -> Option<String> {
        Some(abilities::client_send_line("cast aura of hate"))
    }
}
...
mod tests {
    ...
    fn cause_light_wounds_targeted() {
        let result =
            TriadGuild::cast_cause_light_wounds(&data("cclw", "orc"), &mut empty_ctx()).unwrap();
        assert_eq!(result, "@target orc;cast 'cause light wounds' orc");
    }

    #[test]
    fn damn_armament_weapon2() {
        let result = TriadGuild::cast_damn_armament(&data("cda", ""), &mut empty_ctx()).unwrap();
        assert_eq!(result, "@cast 'damn armament' weapon2");
    }

    #[test]
    fn aura_of_hate_no_quotes() {
        let result = TriadGuild::cast_aura_of_hate(&data("caoh", ""), &mut empty_ctx()).unwrap();
    ...
    fn mellon_collie_targeted() {
        let result =
            TriadGuild::cast_mellon_collie(&data("cmc", "troll"), &mut empty_ctx()).unwrap();
